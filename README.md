...
从 v1.7.1 到 v1.7.2 的主要变化：

- 优化单篇 HTML/Markdown 导出策略：默认导出单文件，只有存在附件类资源时才生成资源 zip，图片资源会尽量内嵌到单文件中。
- 优化 HTML 导出页面样式，补充分栏、折叠块、代码块工具栏和行号等语雀内容结构的离线展示效果。
- 新增多维表格日期/时间范围筛选，支持今天、昨天、本周、上周、本月、上个月、最近7天和最近30天。
- 优化多维表格看板卡片标题区域，减少拖拽图标对标题排版的干扰。
- 更新 README 发版说明，并更新应用、Tauri 配置、Node 包和 Rust crate 版本号到 1.7.2。
