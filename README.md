...
- 新建和导入 `.xlsx` 表格，并使用 Univer 开源 Sheets 在本地编辑、自动保存和另存。
- 支持知识库、目录、文档的层级展示。
...
- 表格编辑器：Univer 开源 Sheets，XLSX 读写使用 ExcelJS
...
│   ├── features/spreadsheet        # Univer 表格编辑和 XLSX 桥接
...
`.lake` 文档和 `.xlsx` 表格保存在用户选择的知识库目录中，例如：
...
说明：知识库 Markdown ZIP 导出只导出 `.lake` 文档；`.xlsx` 表格会保留为知识库中的独立文件，并通过备份恢复链路处理。当前 XLSX 兼容范围覆盖基础单元格值、公式表达式、样式、合并单元格和多 Sheet，不承诺宏、透视表、复杂图表或受保护工作簿的完整保真。
...
2. 新建目录、`.lake` 文档和 `.xlsx` 表格。
3. 编辑文档内容并观察自动保存状态。
4. 新建多个标题，确认 Lake 编辑器内置大纲正常显示。
5. 重命名目录、文档和知识库。
6. 导入一个 `.xlsx`，编辑后另存，确认文件仍为本地 `.xlsx`。
7. 删除测试目录、测试文档或测试表格。
