...
从 v1.7.0 到 v1.7.1 的主要变化：

- 新增 OpenAI Chat Completions 协议配置，兼容更多 OpenAI 接口形态和第三方兼容服务。
- 优化 AI 模型设置页，已配置模型会明确标记当前使用项，并支持删除已配置模型。
- 新增多维表格字段拖拽排序，支持调整字段展示顺序。
- 新增多维表格粘贴导入能力，可从 Excel 复制数据并按表头匹配或创建字段。
- 修复多维表格正文全屏编辑缺少大纲的问题，保持全屏编辑时的文档结构导航。
- 优化多维表格正文编辑和导入相关样式，提升复杂内容录入体验。
- 更新 README 发版说明，并更新应用、Tauri 配置和 Rust crate 版本号到 1.7.1。
