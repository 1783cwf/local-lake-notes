...
const doc = (path: string, parentPath = ""): WorkspaceDocument => ({
  ...
  parentPath,
  kind: path.endsWith(".xlsx") ? "spreadsheet" : "lake",
  size: 1,
});
...
test("从 .xlsx 路径提取表格标题", () => {
  expect(documentTitleFromPath("nested/预算表.xlsx")).toBe("预算表");
});

test("计算文档拖入目录的目标父目录和乐观路径", () => {
  const workspace = workspacePayload(
    [doc("a.xlsx"), doc("notes/b.lake", "notes")],
    [dir("notes")],
  );
  const tree = buildDocumentTree(workspace.documents, workspace.directories, workspace.order);

  const move = resolveWorkspaceMove(tree, "document:a.xlsx", {
    placement: "inside",
